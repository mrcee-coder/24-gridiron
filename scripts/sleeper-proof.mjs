...
const [users,drafts,rosters]=await Promise.all([get(`/league/${league}/users`),get(`/league/${league}/drafts`),get(`/league/${league}/rosters`)]);
...
const rosterMap=(rosters||[]).map(r=>({roster_id:r.roster_id,owner_id:r.owner_id,manager:names[r.owner_id]||null,players:r.players||[],drafted:picksByRoster[r.roster_id]||[]}));
